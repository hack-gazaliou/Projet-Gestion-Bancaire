...
import Modele
from Modele import Compte, Operation
from Modele.Compte import Compte, Decouvert, TypeCompte
...
class Controler : 
        ...
        with SessionLocal() as session:
            ...
            for c in clients_sql:
                liste_affichage.append(
                    {
                        "id": c.customer_id,
                        "nom": f"{c.first_name.capitalize()} {c.last_name.capitalize()}",
                    }
                )
    ...
    def get_comptes_client(
        self, client_id
    ) -> (
        dict
    ):  # Récupère la liste des comptes d'un client et calcule leurs soldes à la volée.
        with SessionLocal() as session:
            try:
                comptes_du_client = (
                    session.query(Compte).filter_by(id_client=client_id).all()
                )  # ne devrait pas arriver pcq on a deja créé la colonne id_clilent, bon pour le debug
            except Exception as e:
            ...
            for compte in comptes_du_client:
                ...
                data_comptes.append(
                    {"id": compte.id, "type": nom_type, "solde": solde_formate}
                )
            return data_comptes


    def gerer_operation_espece(
        compte_id, montant, type_operation: TypeOperation
    ):  # enum de la classe opération: DEPOT = 0 /RETRAIT=1
        solde_init = (Compte.obtenir(compte_id)).solde
        match type_operation.value:
            case 0:
                Operation.transferer(
                    0, compte_id, montant
                )  # source = compte, cible =banque
                solde_act = (Compte.obtenir(compte_id)).solde
                return (
                    True,
                    f"Le depot de {montant / 100} € a bien été effectué. Solde actuel : {solde_act / 100} €.",
                )
            case 1:
                if solde_init - montant > -DECOUVERT_MAX:
                    ...
                    return (
                        True,
                        f"Le retrait de {montant / 100}€ a bien été effectué. Solde actuel : {solde_act / 100}.",
                    )
                else:
                    return (
                        False,
                        f"Solde insuffisant pour effectuer l'opération.\nSolde = {solde_init / 100}, montant du retrait = {montant / 100}. \n Découvert max autorisé : {DECOUVERT_MAX / 100}.",
                    )
    ...
    def effectuer_virement(id_source, id_cible, montant):
        ...
        if solde_init - montant > -DECOUVERT_MAX:
            ...
            return (
                True,
                f"Virement de {montant / 100} € effectué, solde actuel : {solde_act / 100} €.",
            )
        else:
            return (
                False,
